...
class So_Pmi:
	def __init__(self):
		...
		self.posword_path = "pos_words_withoutph.txt"
		self.negword_path = "neg_words_withoutph.txt"
		self.pos_pic_path = "pos_words_withoutph.png"
		self.neg_pic_path = "neg_words_withoutph.png"
		self.seed_path = "seed_words.txt"

	#分词
	def cut_words(self, cmt_path):
	...
	def collect_frq(self, seg_data, cowords_list, seed_path):
		...
		def collect_cowordsdict(cowords_list):
			...
			for co_words in cowords_list:
				candi_words.append(co_words.split('\t')[1])		#只把非种子词也加入到候选词中去
	...
	def get_pmi_dict(self, candi_words, seed_path, word_dict, co_dict, num_of_allwords):
		...
		def compute_pmi(p1, p2, p12):
			...
			#return math.log2(p12 + 0.00001) - math.log2(p1 * p2 + 0.00001)
	...
	def save_res(self, pmi_dict, posword_path, negword_path, pos_pic_path, neg_pic_path):
		...
		return finish_txt, finish_png	#返回执行时间，作为运行的参考

	def so_pmi(self):
		start_time = time.time()
		seg_data = self.cut_words(self.cmt_path)	#完成分词，舍去了出现率较高的标点和无倾向性的单字
		...
		txt_end, png_end = self.save_res(pmi_dict, self.posword_path, self.negword_path, self.pos_pic_path, self.neg_pic_path)
						#排序并得出结果
		print("With Irrelevant Words: Time of Finishing So_Pmi = {0}s".format(txt_end - start_time))
